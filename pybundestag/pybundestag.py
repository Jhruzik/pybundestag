...
if __name__ == "__main__":
    ...
    args.output = "/home/joshuahruzik/mdbs.csv"
    args.seperator = ";"
    args.meta = True
    ...
    if args.entity == "protocol":
        ...
        # Parse if Input is a Folder
            ...
            for file in content:
                ...
                speeches_tmp = parser.speechparser.collect_speeches(protocol, output = "list", metadata = args.meta)
                speeches_list.extend(speeches_tmp)
            # Write Result as CSV to Output Path
            if output_extension == "csv":
                result_df = pd.DataFrame(speeches_list)
                result_df.to_csv(args.output, sep = args.seperator, encoding = "utf-8", index = False)
